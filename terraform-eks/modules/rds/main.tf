...
resource "aws_vpc" "db" {
  ...
  tags                 = {
    # Name = "${var.project}-${var.version_name}-db-vpc"
    Name = "${var.project}-db-vpc"
  }
}
...
resource "aws_internet_gateway" "db-igw" {
  vpc_id = aws_vpc.db.id
  tags   = {
    # Name = "cc-${var.version_name}-db-igw"
    Name = "cc-db-igw"
  }
}
...
resource "aws_route_table" "db_route_table" {
  vpc_id = aws_vpc.db.id
  tags   = {
    # Name = "cc-${var.version_name}-db-route-table"
...
resource "aws_secretsmanager_secret" "db-pass" {
  # name = "cc-${var.version_name}-db-pass-${random_id.id.hex}"
  name = "cc-db-pass"
...
resource "aws_db_instance" "rds" {
  # identifier             = "${var.project}-${var.version_name}-database"
  identifier             = "${var.project}-database"
  allocated_storage      = var.allocated_storage
  engine                 = "mysql"
  engine_version         = var.engine_version
  availability_zone      = var.availability_zones[0] ######  CHANGE!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  secret_string = jsonencode(
    {
      ...
      engine   = "mysql"
      host     = aws_db_instance.rds.address # aws_db_instance.rds.endpoint
    }
  )
}
